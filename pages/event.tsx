      ...
      <div className="container mx-auto p-4">
        ...
        <div className="mb-6 flex flex-wrap gap-6 justify-center">
          ...
          >
            ...
            <option value="Chareonkrung-Talad Noi">
              Chareonkrung-Talad Noi
            </option>
            <option value="Klongtoey">Klongteoy</option>
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {filteredEvents.length > 0 ? (
            filteredEvents.map((event) => {
              const imagePath = `${API_URL}/public/${
                event.images?.[0] || "default.jpg"
              }`; // Use default image if none
              return (
                // Add the return statement here
                <div
                  key={event._id}
                  className="group relative bg-white rounded-lg shadow-lg overflow-hidden transform hover:scale-105 hover:shadow-2xl transition-all duration-300"
                >
                  <div className="relative">
                    ...
                    <div className="absolute top-0 right-0 p-4 text-white bg-gradient-to-t via-transparent to-transparent rounded-bl-lg">
                      <span className="bg-indigo-600 text-sm font-medium py-1 px-3 rounded">
                        {event.type}
                      </span>
                    </div>
                  </div>
